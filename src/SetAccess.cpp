...
BOOL AddAccessRights(TCHAR *lpszFileName, TCHAR *lpszAccountName,DWORD dwAccessMask) 
{
	...
	LPVOID
		...
		= NULL;
	UINT
		CurrentAceIndex = 0; //ACE在ACL中的位置
	UINT
		newAceIndex = 0; //新添的ACE在ACL中的位置
	...
	__try {
		...
		//
		//STEP14:把一个access-allowed的ACE加入到新的DACL中
		//
		...
		//在Windows2000以后的版本支持，NT则没有，我们为了使用新版本的函数，我们首
		...
		//
		//STEP15:按照已存在的ACE的顺序拷贝从父目录继承而来的ACE
		//
		if(fDaclPresent&&AclInfo.AceCount){
			for(;
				CurrentAceIndex<AclInfo.AceCount;
		...
		//
		//STEP19:把老的SD中的控制标记再拷贝到新的SD中，我们使用的是一个叫
		//SetSecurityDescriptorControl()的API函数，这个函数同样只存在于
		//Windows2000以后的版本中，所以我们还是要动态地把其从advapi32.dll
		...
		//
		//STEP20:把新的SD设置设置到文件的安全属性中（千山万水啊，终于到了）
		//
...
{
	if(argc<3){
		_tprintf(TEXT("usage:%s <FileName> <AccountName> "), argv[0]);
		return 1;
	}
	//argv[1]–文件（目录）名
	//argv[2]–用户（组）名
